...
(function () {
  ...
  async function loadActiveTempCorrections(sb, logRows) {
    const rows = Array.isArray(logRows) ? logRows : [];
    const logIds = rows.map(row => String(row?.log_id || '').trim()).filter(Boolean);
    const map = new Map();
    if (!logIds.length) return map;
    const { data, error } = await sb.from('temp_log_corrections')
      .select('*')
      .in('original_log_id', logIds)
      .eq('is_active', true)
      .order('corrected_at', { ascending: false });
    if (error) {
      const text = String(error.message || error.details || error);
      if (/temp_log_corrections|does not exist|schema cache|PGRST/i.test(text)) return map;
      throw error;
    }
    (data || []).forEach(row => {
      const key = String(row.original_log_id || '').trim();
      if (key && !map.has(key)) map.set(key, row);
    });
    return map;
  }

  function correctedLogView(log, fridge, correction) {
    const originalIsNoTemp = String(log?.record_type || '').toUpperCase() === 'NO_TEMP';
    const correctedTemp = correction && correction.corrected_temp !== null && correction.corrected_temp !== ''
      ? Number(correction.corrected_temp)
      : null;
    const hasCorrection = Number.isFinite(correctedTemp);
    const rawTemp = originalIsNoTemp || log?.temp === null || log?.temp === '' ? null : Number(log.temp);
    const effectiveTemp = hasCorrection ? correctedTemp : rawTemp;
    let effectiveStatus = log?.status || '';
    if (hasCorrection) {
      const range = getTempRange(fridge || {});
      effectiveStatus = range && (effectiveTemp < range.minTemp || effectiveTemp > range.maxTemp) ? 'ผิดปกติ' : 'ปกติ';
    }
    return {
      hasCorrection,
      effectiveTemp,
      effectiveStatus,
      effectiveRecordType: hasCorrection ? 'TEMP' : (log?.record_type || 'TEMP'),
      originalIsNoTemp
    };
  }

  function buildRecordedItem(log, fridge, correction = null) {
    const view = correctedLogView(log, fridge, correction);
    const isAutoIncident = !view.hasCorrection && log.auto_generated === true && !!log.related_incident_id;
    return {
      ...
      latestTemp: view.effectiveTemp === null || !Number.isFinite(Number(view.effectiveTemp)) ? '' : view.effectiveTemp,
      latestTempText: view.effectiveRecordType === 'NO_TEMP' ? 'ไม่สามารถวัดได้' : '',
      latestStatus: view.effectiveStatus,
      latestAction: view.hasCorrection ? `แก้ไขย้อนหลังเป็น ${view.effectiveTemp} °C` : (log.action_text || ''),
      recorderName: resolveStaffFullNameCached(log.recorder_name),
      recordType: view.effectiveRecordType,
      relatedIncidentId: log.related_incident_id || '',
      autoGenerated: log.auto_generated === true,
      hasCorrection: view.hasCorrection,
  ...
  async function dashboardSummary(params) {
    ...
    (logRows || []).forEach(log => {
      ...
      const correction = correctionMap.get(String(log.log_id || '').trim()) || null;
      if (log.round === 'เช้า') morningMap.set(log.fridge_id, buildRecordedItem(log, fridge, correction));
      if (log.round === 'เย็น') eveningMap.set(log.fridge_id, buildRecordedItem(log, fridge, correction));
    });
  ...
  async function submitTemperature(params) {
    ...
    if (needIncidentAlert) {
      ...
      // V1.8.43: ผูก LOG กับ Incident ที่สร้าง/ใช้อยู่จริงทันที
      // เพื่อให้ KPI และการแก้ไขย้อนหลังไม่ต้องเดาความสัมพันธ์จากวันที่
      if (incidentId) {
        const { error: linkErr } = await sb.rpc('temp_link_log_incident_v1843', {
          p_log_id: logId,
          p_incident_id: incidentId
        });
        if (linkErr) console.warn('link temp log to incident failed:', linkErr);
      }
    }
  ...
  async function todayLogStatus(params) {
    ...
    if (error) throw error;
    const rows = data || [];
    const correctionMap = await loadActiveTempCorrections(sb, rows);
    const fridgeIds = [...new Set(rows.map(row => String(row.fridge_id || '').trim()).filter(Boolean))];
    const fridgeMap = new Map();
    if (fridgeIds.length) {
      const { data: fridgeRows, error: fridgeError } = await sb.from('temp_fridges')
        .select('fridge_id,min_temp,max_temp')
        .in('fridge_id', fridgeIds);
      if (fridgeError) throw fridgeError;
      (fridgeRows || []).forEach(row => fridgeMap.set(String(row.fridge_id || '').trim(), row));
    }
    return { ok: true, date, fridgeId, records: rows.map(row => {
      const correction = correctionMap.get(String(row.log_id || '').trim()) || null;
      const view = correctedLogView(row, fridgeMap.get(String(row.fridge_id || '').trim()) || null, correction);
      return {
        date: row.log_date,
        round: row.round,
        time: normalizeTime(row.log_time),
        fridgeId: row.fridge_id,
        temp: view.effectiveTemp,
  ...
  async function getHistory(params) {
    ...
    const logIds = sourceRows.map(row => String(row.log_id || '').trim()).filter(Boolean);
    const correctionMap = new Map();
    if (logIds.length) {
      const { data: correctionRows, error: correctionError } = await sb.from('temp_log_corrections')
        .select('*')
        .in('original_log_id', logIds)
        .eq('is_active', true)
        .order('corrected_at', { ascending: false });
      if (correctionError) {
        const text = String(correctionError.message || correctionError.details || correctionError);
        // รองรับฐานที่ยังไม่ได้ Run SQL V1.8.43 โดยหน้า history เดิมยังเปิดได้
        if (!/temp_log_corrections|does not exist|schema cache|PGRST/i.test(text)) throw correctionError;
      } else {
        (correctionRows || []).forEach(row => {
          const key = String(row.original_log_id || '').trim();
          if (key && !correctionMap.has(key)) correctionMap.set(key, row);
        });
      }
    }
    ...
    const records = sourceRows.map(row => {
      const correction = correctionMap.get(String(row.log_id || '').trim()) || null;
      const originalTemp = row.record_type === 'NO_TEMP' || row.temp === null || row.temp === '' ? null : Number(row.temp);
      const correctedTemp = correction && correction.corrected_temp !== null && correction.corrected_temp !== ''
        ? Number(correction.corrected_temp)
        : null;
      const effectiveTemp = correction ? correctedTemp : originalTemp;
      let effectiveStatus = row.status || '';
      if (correction && effectiveTemp !== null && Number.isFinite(effectiveTemp)) {
        effectiveStatus = range && (effectiveTemp < range.minTemp || effectiveTemp > range.maxTemp) ? 'ผิดปกติ' : 'ปกติ';
      }
      return {
        logId: row.log_id || '',
        ...
        temp: effectiveTemp,
        tempDisplay: effectiveTemp === null || !Number.isFinite(Number(effectiveTemp)) ? '-' : String(effectiveTemp),
        originalTemp,
        originalTempDisplay: originalTemp === null || !Number.isFinite(Number(originalTemp)) ? '-' : String(originalTemp),
        correctedTemp,
        ...
        recorderName: resolveStaffFullNameCached(row.recorder_name),
        recordType: correction ? 'TEMP' : (row.record_type || 'TEMP'),
        originalRecordType: row.record_type || 'TEMP',
        isValidForGraph: correction ? true : (row.is_valid_for_graph !== false && row.record_type !== 'NO_TEMP'),
        ...
        correctionId: correction?.correction_id || null,
  ...
  async function reviewKpiLog(params) {
    ...
    const classification = String(params.get('classification') || '').trim();
    const reason = String(params.get('reason') || '').trim();
    const reviewedBy = String(params.get('reviewedBy') || '').trim();
    if (!logId || !classification || !reason || !reviewedBy) return { ok: false, message: 'กรุณากรอกข้อมูลการทบทวนให้ครบ' };
    const { data, error } = await sb.rpc('temp_review_kpi_log_v1843', {
      p_log_id: logId,
  ...
  async function correctTempLog(params) {
    const sb = getClient();
    const logId = String(params.get('logId') || '').trim();
  ...
  async function handleUrl(urlText, init = {}) {
    try {
      ...
      else if (action === 'review_kpi_log') payload = await reviewKpiLog(params);
      else if (action === 'temp_correct_log') payload = await correctTempLog(params);
